    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <Input id="departamento" value="Caldas" disabled className="bg-gray-100" />
        </div>
        <div>
          ...
          <Select 
            ...
            required
          >
            <SelectTrigger id="municipio"><SelectValue placeholder="Seleccione un municipio" /></SelectTrigger>
            <SelectContent>
      ...
      <div className="flex justify-end space-x-2">
        ...
        <Button type="submit">
          <Save className="h-4 w-4 mr-2" /> {votante ? 'Actualizar' : 'Guardar'}
        </Button>
      </div>
    </form>
  )
...
function MensajeriaMasiva({ votantes }: { votantes: Votante[] }) {
    ...
    try {
      ...
      if (plataforma === 'sms') {
        ...
        if (response.ok) {
          ...
          alert(`✅ SMS enviados: ${result.enviados || selectedVotantes.length} de ${selectedVotantes.length}`)
          setSelectedVotantes([])
          setMensaje('')
        } else {
          const error = await response.json()
          alert(error.error || 'Error al enviar SMS')
        }
      }
  ...
  const selectAllVotantes = () => setSelectedVotantes(votantes.map(v => v.id))
  const clearSelection = () => setSelectedVotantes([])
        ...
        <Tabs defaultValue="compose" className="w-full">
          ...
          <TabsContent value="compose" className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                <MessageSquare className="h-6 w-6 mb-2" /> WhatsApp
              </Button>
              <Button 
                ...
                className="h-16 flex-col"
              >
                <Mail className="h-6 w-6 mb-2" /> Email
              </Button>
              <Button 
                ...
                className="h-16 flex-col"
              >
                <Phone className="h-6 w-6 mb-2" /> SMS
              </Button>
              <Button 
                ...
                className="h-16 flex-col"
              >
                <Users className="h-6 w-6 mb-2" /> Instagram
              </Button>
            </div>
          ...
          <TabsContent value="select" className="space-y-4">
            <div className="flex justify-between items-center">
              <h4 className="font-medium">Seleccionados: {selectedVotantes.length} de {votantes.length}</h4>
              <div className="space-x-2">
                <Button variant="outline" size="sm" onClick={selectAllVotantes}>Seleccionar Todos</Button>
            ...
            <div className="max-h-60 overflow-y-auto border rounded-lg">
              {votantes.map((votante) => (
                <div key={votante.id} className="flex items-center space-x-3 p-3 hover:bg-gray-50 border-b">
                  <input
                    type="checkbox"
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">{votante.municipio} - Barrio {votante.barrio}</p>
                  </div>
                  <Badge variant={votante.estado === 'simpatizante' ? 'default' : votante.estado === 'voluntario' ? 'secondary' : 'outline'}>
                    {votante.estado}
                  </Badge>
                </div>
              ))}
...
export default function VotantesManager({ votantes, onVotanteChange }: VotantesManagerProps) {
  ...
  const votantesFiltrados = votantes.filter(votante => {
    const coincideCedula = !filtroCedula || votante.cedula.toLowerCase().includes(filtroCedula.toLowerCase())
    const coincideEstado = filtroEstado === 'todos' || votante.estado === filtroEstado
    const coincideMunicipio = filtroMunicipio === 'todos' || votante.municipio === filtroMunicipio
  ...
  const handleSave = async (votanteData: Partial<Votante>) => {
    setLoading(true)
    try {
      const url = editingVotante ? `/api/votantes/${editingVotante.id}` : '/api/votantes'
  ...
  const handleDelete = async (votanteId: string) => {
    if (!confirm('¿Estás seguro de que quieres eliminar este votante?')) return
    setLoading(true)
    try {
      const response = await fetch(`/api/votantes/${votanteId}`, { method: 'DELETE' })
      if (response.ok) {
        onVotanteChange()
      } else {
        const error = await response.json()
  ...
  const handleEdit = (votante: Votante) => { setEditingVotante(votante); setShowForm(true) }
  const handleNew = () => { setEditingVotante(undefined); setShowForm(true) }
  const handleClose = () => { setShowForm(false); setEditingVotante(undefined) }
    ...
    <div>
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex space-x-3">
            ...
            <Button className="bg-blue-600 hover:bg-blue-700" onClick={handleNew}>
              <Users className="h-4 w-4 mr-2" /> Agregar Votante
            </Button>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full">
            <div className="w-full sm:w-48">
              ...
              <Select value={filtroEstado} onValueChange={setFiltroEstado}>
                <SelectTrigger id="filtroEstado" className="mt-1"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="w-full sm:w-48">
              ...
              <Select value={filtroMunicipio} onValueChange={setFiltroMunicipio}>
                <SelectTrigger id="filtroMunicipio" className="mt-1"><SelectValue /></SelectTrigger>
                <SelectContent>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {votantesFiltrados.map((votante) => (
                      <tr key={votante.id} className="hover:bg-blue-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <Badge 
                            variant={votante.estado === 'simpatizante' ? 'default' : votante.estado === 'voluntario' ? 'secondary' : 'outline'}
                            className={votante.estado === 'simpatizante' ? 'bg-green-100 text-green-800' : votante.estado === 'voluntario' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800'}
                          >
                            {votante.estado}
                          </Badge>
                        </td>
        ...
        <TabsContent value="importar">
          <Card>
            ...
            <CardContent>
              <p className="text-gray-600">Funcionalidad de importación disponible</p>
            </CardContent>
          </Card>
        </TabsContent>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingVotante ? 'Editar Votante' : 'Agregar Nuevo Votante'}</DialogTitle>
            <DialogDescription>
              {editingVotante ? 'Edita la información del votante seleccionado' : 'Completa el formulario para agregar un nuevo votante'}
            </DialogDescription>
          </DialogHeader>
          <VotanteForm votante={editingVotante} onSave={handleSave} onCancel={handleClose} />
        </DialogContent>
      </Dialog>
    </div>
  )
